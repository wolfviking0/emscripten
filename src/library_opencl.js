...
, "");
    ...
    /**
     * Description
     * @method getCopyPointerToArrayPowTwo
     * @param {} ptr
     * @param {} size
     * @param {} type
     * @return _host_ptr
      ...
      if (type.length == 1) {
        switch(type[0][0]) {
          case webcl.SIGNED_INT8:
            ...
            _host_ptr.set( {{{ makeHEAPView('8','ptr','ptr+size') }}} );
            break;
          case webcl.SIGNED_INT16:
            var _size = size >> 1;
            var _offset = CL.getNextPowOfTwo(_size);
            _host_ptr = new Int16Array(_offset);
            _host_ptr.set( {{{ makeHEAPView('16','ptr','ptr+size') }}} );
            break;
          case webcl.SIGNED_INT32:
            var _size = size >> 2;
            var _offset = CL.getNextPowOfTwo(_size);
            _host_ptr = new Int32Array(_offset);
            _host_ptr.set( {{{ makeHEAPView('32','ptr','ptr+size') }}} );
            break;
          case webcl.UNSIGNED_INT8:
            var _size = size;
            var _offset = CL.getNextPowOfTwo(_size);
            _host_ptr = new Uint8Array(_offset);
            _host_ptr.set( {{{ makeHEAPView('U8','ptr','ptr+size') }}} );
            break;
          case webcl.UNSIGNED_INT16:
            var _size = size >> 1;
            var _offset = CL.getNextPowOfTwo(_size);
            _host_ptr = new Uint16Array(_offset);
            _host_ptr.set( {{{ makeHEAPView('U16','ptr','ptr+size') }}} );
            break;
          case webcl.UNSIGNED_INT32:
            var _size = size >> 2;
            var _offset = CL.getNextPowOfTwo(_size);
            _host_ptr = new Uint32Array(_offset);
            _host_ptr.set( {{{ makeHEAPView('U32','ptr','ptr+size') }}} );
            break;
          default:
            var _size = size >> 2;
            var _offset = CL.getNextPowOfTwo(_size);
            _host_ptr = new Float32Array(_offset);
            _host_ptr.set( {{{ makeHEAPView('F32','ptr','ptr+size') }}} );
            break;
        }
      } else {
        var _size = size >> 2;
    ...
    copyDataToHeap: function(dest, src, size, type) {
      ...
      if (type.length == 0) {
#if CL_DEBUG
        console.error("copyDataToHeap : error unknow type with length null "+type);
#endif
      }
    ...
    webclBeginStackTrace: function(name,parameter) {
      if (CL.stack_trace_offset == -1) {
        CL.stack_timer = new Date().getTime();
    ...
    webclEndStackTrace: function(result,message,exception) {
      var stack_elapsed = new Date().getTime() - CL.stack_timer;
      ...
      if (CL.stack_trace_offset == "") {
        CL.stack_trace += " - Elapsed (" + stack_elapsed + " ms )\n";
      } else {
        CL.stack_trace += "\n";
      }
...
#if CL_GRAB_TRACE
        CL.webclCallStackTrace( CL.cl_objects[context]+".createImage",[_flags,"{channelOrder:"+_channel_order+", channelType:"+_channel_type+", width:"+image_width+", height:"+image_height+", rowPitch:"+image_row_pitch +"}"]);
#endif
  ...
  clGetImageInfo: function(image,param_name,param_value_size,param_value,param_value_size_ret) {
    ...
    try {
      ...
      _sizeType = CL.getImageSizeType(image) * CL.getImageSizeOrder(image);
...
#if CL_GRAB_TRACE
        CL.webclCallStackTrace(CL.cl_objects[program]+".getInfo",[webcl.PROGRAM_NUM_DEVICES]);
#endif
  ...
  clGetEventProfilingInfo: function(event,param_name,param_value_size,param_value,param_value_size_ret) {
    ...
    if (!(event in CL.cl_objects)) {
      ...
      if (param_value != 0) {{{ makeSetValue('param_value', '0', '0', 'i64') }}};
      if (param_value_size_ret != 0) {{{ makeSetValue('param_value_size_ret', '0', '8', 'i32') }}};
  ...
  clEnqueueReadBuffer: function(command_queue,buffer,blocking_read,offset,cb,ptr,num_events_in_wait_list,event_wait_list,event) {
    ...
    try {
      ...
      CL.webclCallStackTrace("(*)"+CL.cl_objects[command_queue]+".enqueueReadBuffer",[CL.cl_objects[buffer],_block,offset,cb,_host_ptr,_event_wait_list,_event]);
#endif
  ...
  clEnqueueReadBufferRect: function(command_queue,buffer,blocking_read,buffer_origin,host_origin,region,buffer_row_pitch,buffer_slice_pitch,host_row_pitch,host_slice_pitch,ptr,num_events_in_wait_list,event_wait_list,event) {
    ...
    var _size = _region.reduce(function (a, b) { return a * b; });
    var _host_ptr = CL.getHostPtrArray(size,CL.cl_pn_type);
...
#if CL_GRAB_TRACE
      CL.webclCallStackTrace(""+CL.cl_objects[command_queue]+".enqueueReadBufferRect",[CL.cl_objects[buffer],_block,_buffer_origin,_host_origin,_region,buffer_row_pitch,buffer_slice_pitch,host_row_pitch,host_slice_pitch,_host_ptr,_event_wait_list,_event]);
#endif
      CL.cl_objects[command_queue].enqueueReadBufferRect(CL.cl_objects[buffer],_block,_buffer_origin,_host_origin,_region,buffer_row_pitch,buffer_slice_pitch,host_row_pitch,host_slice_pitch,_host_ptr,_event_wait_list,_event);

      // Copy array to heap
      CL.copyDataToHeap(ptr,_host_ptr.buffer,_size,CL.cl_pn_type);
...
#if CL_GRAB_TRACE
      CL.webclCallStackTrace(""+CL.cl_objects[command_queue]+".enqueueWriteBuffer",[CL.cl_objects[buffer],_block,offset,cb,_host_ptr,_event_wait_list,_event]);
#endif
...
#if CL_GRAB_TRACE
      CL.webclCallStackTrace(""+CL.cl_objects[command_queue]+".enqueueWriteBufferRect",[CL.cl_objects[buffer],_block,_buffer_origin,_host_origin,_region,buffer_row_pitch,buffer_slice_pitch,host_row_pitch,host_slice_pitch,_host_ptr,_event_wait_list,_event]);
#endif
  ...
  clEnqueueReadImage: function(command_queue,image,blocking_read,origin,region,row_pitch,slice_pitch,ptr,num_events_in_wait_list,event_wait_list,event) {
    ...
    try {
      ...
      var _size = CL.getImageSizeType(image) * CL.getImageSizeOrder(image);
      ...
      var _host_ptr = CL.getHostPtrArray(_size,[[_channel,1]]);
      ...
      CL.cl_objects[command_queue].enqueueReadImage(CL.cl_objects[image],_block,_origin,_region,row_pitch,_host_ptr,_event_wait_list, _event);

      if (event != 0) {
        {{{ makeSetValue('event', '0', 'CL.udid(_event)', 'i32') }}};
      }

      CL.copyDataToHeap(ptr,_host_ptr.buffer,_size,[[_channel,1]]);

#if CL_GRAB_TRACE
      CL.webclCallStackTrace(""+CL.cl_objects[command_queue]+".enqueueReadImage",[CL.cl_objects[image],_block,_origin,_region,row_pitch,_host_ptr,_event_wait_list,_event]);
#endif
  ...
  clEnqueueWriteImage: function(command_queue,image,blocking_write,origin,region,input_row_pitch,input_slice_pitch,ptr,num_events_in_wait_list,event_wait_list,event) {
    ...
    try {
      ...
      var _host_ptr = CL.getCopyPointerToArray(ptr,_size,[[_channel,1]]);
...
#if CL_GRAB_TRACE
      CL.webclCallStackTrace(""+CL.cl_objects[command_queue]+".enqueueWriteImage",[CL.cl_objects[image],_block,_origin,_region,input_row_pitch,_host_ptr,_event_wait_list,_event]);
#endif
      ...
      CL.cl_objects[command_queue].enqueueWriteImage(CL.cl_objects[image],_block,_origin,_region,input_row_pitch,_host_ptr,_event_wait_list,_event);
